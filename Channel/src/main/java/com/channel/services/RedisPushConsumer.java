...
public class RedisPushConsumer implements MessageListener {
    Logger logger = Logger.getLogger(RestConsumer.class.getName());
    RestConsumer restConsumer;


    @Autowired
    public RedisPushConsumer(RestConsumer restConsumer) {
    ...
    public void onMessage(Message message, byte[] pattern) {
        logger.log(Level.INFO, " channel: push , message:" + message.toString());
