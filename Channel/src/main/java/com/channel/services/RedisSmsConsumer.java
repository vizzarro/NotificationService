...
public class RedisSmsConsumer implements MessageListener {
    Logger logger = Logger.getLogger(RestConsumer.class.getName());
    RestConsumer restConsumer;


    @Autowired
    public RedisSmsConsumer(RestConsumer restConsumer){
    ...
    public void onMessage(Message message, byte[] pattern) {
        logger.log(Level.INFO," channel: sms , message:"+message.toString());
